...
4. [Sistema de Temas — 9 Temas (3 FREE + 6 PRO)](#4-sistema-de-temas)
...
- **TEMA** = aparência visual (cores). 9 opções. Configuração pessoal. Não vinculado a plano (FREE tem 3, PRO tem 9).
...
- Definição dos 9 temas com tokens CSS
...
| 7 | `graphite` | Graphite | Dark | PRO | Silver `#a0a0b8` | Neutro universal, aço escovado |
| 8 | `twilight` | Twilight | Dark | PRO | Violeta `#8b5cf6` | Criativo, inovador |
| 9 | `sahara` | Sahara | Light | PRO | Terracota `#c2703e` | Acolhedor orgânico, lifestyle |
...
- **RN-TEMA-02:** Temas PRO (obsidian, rosewood, arctic, graphite, twilight, sahara) só podem ser ativados por usuários com plano PRO.
...
Esses tokens são idênticos em todos os 9 temas:
...
[data-theme="graphite"] {
  ...
  --border-h: rgba(180, 180, 200, 0.16);
  
  /* ACENTO: Silver monocromático — neutro universal */
  --accent:     #a0a0b8;
  --accent-rgb: 160, 160, 184;
  --accent2:    #c0c0d0;
  
  --module-bar-bg: #0c0c0f;
  --sidebar-bg:    #141418;
  --header-bg:     rgba(17, 17, 20, 0.90);
  
  --card-shadow: 0 2px 16px rgba(0, 0, 0, 0.45);
  --glass:       rgba(26, 26, 30, 0.80);
  
  color-scheme: dark;
}
```

### 5.9 Tema 8: Twilight (PRO)
...
[data-theme="twilight"] {
  ...
  --border-h: rgba(139, 92, 246, 0.18);
  
  /* ACENTO: Violeta — criativo e inovador */
  --accent:     #8b5cf6;
  --accent-rgb: 139, 92, 246;
  --accent2:    #a78bfa;
  
  --module-bar-bg: #08060e;
  --sidebar-bg:    #100d1a;
  --header-bg:     rgba(12, 10, 20, 0.88);
  
  --card-shadow: 0 2px 16px rgba(0, 0, 0, 0.50);
  --glass:       rgba(20, 17, 31, 0.75);
  
  color-scheme: dark;
}
```

### 5.10 Tema 9: Sahara (PRO)
...
[data-theme="sahara"] {
  ...
  --border:   rgba(42, 32, 24, 0.07);
  --border-h: rgba(42, 32, 24, 0.13);
  
  /* ACENTO: Terracota — quente e orgânico */
  --accent:     #c2703e;
  --accent-rgb: 194, 112, 62;
  --accent2:    #d4946a;
  
  --module-bar-bg: linear-gradient(180deg, #3d2b1f, #5c3d2e);
  --sidebar-bg:    #faf6f0;
  --header-bg:     rgba(245, 240, 232, 0.88);
  
  --card-shadow: 0 2px 12px rgba(60, 40, 20, 0.06);
  --glass:       rgba(255, 252, 246, 0.70);
  
  color-scheme: light;
}
```

### 5.11 Relação --accent vs --em vs cores de módulo

Para evitar confusão, aqui está exatamente o que cada token controla:

| Token | Controla | Muda por tema? |
|---|---|---|
| `--accent` | Cor de acento genérica da UI: botões primários, links, toggles ativos, barras de progresso genéricas, badge ativo na sidebar genérica | ✅ SIM — dourado no Obsidian, rose gold no Rosewood, cyan no Arctic, silver no Graphite, violeta no Twilight, terracota no Sahara |
...
src/
  styles/
    themes.css          ← definições de todos os 9 temas
...
type ThemeId = 'navy-dark' | 'clean-light' | 'mint-garden' | 'obsidian' | 'rosewood' | 'arctic' | 'graphite' | 'twilight' | 'sahara' | 'system';
...
```typescript
const PRO_THEMES: ThemeId[] = ['obsidian', 'rosewood', 'arctic', 'graphite', 'twilight', 'sahara'];
...
```
synclife-theme = 'navy-dark' | 'clean-light' | 'mint-garden' | 'obsidian' | 'rosewood' | 'arctic' | 'graphite' | 'twilight' | 'sahara' | 'system'
...
| **16-GUIA-CRIACAO-SPEC-DE-TELAS.md** | Seção 2.4 "Quatro combinações de tema obrigatórias" | Reescrever: de "4 combinações" para "9 temas × 2 modos = 18 combinações teóricas, mas temas são automáticos via tokens" |
...
| **17-NAVEGACAO-SHELL-DEV-SPEC.md** | Seção 14 "Toggle de Tema" | Reescrever: de ThemePill (Dark/Light toggle) para link "Aparência" nas Configurações, ou manter pill mas com seleção dos 9 temas. |
| **17-NAVEGACAO-SHELL-DEV-SPEC.md** | Seção 18 "Quatro Combinações Visuais" | Reescrever: de 4 para 9 temas, e explicar que modo é funcional. |
...
### 17.1 Testes de temas (24 testes)
...
describe('Theme System')
  ...
  ✓ deve aplicar tokens CSS corretos para graphite
  ✓ deve aplicar tokens CSS corretos para twilight
  ✓ deve aplicar tokens CSS corretos para sahara
  ...
  ✓ deve manter cores funcionais (--green, --red) iguais em todos os 9 temas
  ✓ deve manter cores de módulo (--fin, --meta) iguais em todos os 9 temas
  ✓ deve aplicar --accent diferente para cada tema PRO (dourado, rose gold, cyan, silver, violeta, terracota)
  ✓ deve manter --em fixo (#10b981) mesmo em temas PRO
  ✓ deve manter --el fixo (#0055ff) mesmo em temas PRO
```
...
```
1.1  Criar src/styles/themes.css com os 9 temas + tokens comuns
...
```
6.1  52 testes unitários conforme seção 17
6.2  Teste visual: cada tema (9) em tela de Finanças Dashboard
