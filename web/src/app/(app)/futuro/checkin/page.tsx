...
function getStatusPillStyles(status: 'on_track' | 'attention' | 'at_risk') {
  switch (status) {
    case 'on_track': return { bg: 'rgba(31,166,122,0.10)', color: 'var(--sl-success)' }
    case 'attention': return { bg: 'rgba(217,150,46,0.10)', color: 'var(--sl-warning)' }
    case 'at_risk': return { bg: 'rgba(219,100,120,0.10)', color: 'var(--sl-danger)' }
  }
}
...
function getMoodColor(value: number): string {
  if (value <= 3) return 'var(--sl-danger)'
  if (value <= 6) return 'var(--sl-warning)'
    ...
    if (!loading && active.length > 0) {
      return active.slice(0, 4).map((obj, idx): CheckinGoal => {
        ...
        return {
          ...
          iconColor: idx % 2 === 0 ? 'var(--sl-danger)' : 'var(--sl-em)',
          status,
          statusLabel,
      ...
      <div className="lg:hidden min-h-screen bg-[var(--sl-bg)] flex flex-col pb-[calc(68px+16px)]">
        ...
        <div className="flex items-center gap-3 px-5 pt-[14px] pb-4 border-b border-[var(--sl-border)]">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-[var(--sl-t3)] mt-[1px]">
              Semana {getCurrentWeekNumber()} · {getCurrentDateString()}
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 px-5 pt-5 flex flex-col gap-4">
          ...
          <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[16px] p-4 relative overflow-hidden">
            <div className="absolute top-0 left-4 right-4 h-[2px] rounded-b-sm" style={{ background: MOD_FUTURO }} />
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)] mb-3 flex items-center gap-2">
              <Smile size={14} style={{ color: MOD_FUTURO }} />
            ...
            <input
              ...
              style={{
                background: `linear-gradient(to right, rgba(219,100,120,0.25), rgba(217,150,46,0.25), rgba(31,166,122,0.25))`,
              }}
            />
            <div className="flex justify-between text-[10px] text-[var(--sl-t3)]">
              <span className="flex items-center gap-1"><Frown size={12} className="text-[var(--sl-danger)]" />Ruim</span>
              <span className="sl-num" style={{ color: moodColor }}>{mood}/10</span>
              <span className="flex items-center gap-1"><Smile size={12} className="text-[var(--sl-success)]" />Ótimo</span>
            </div>
          </div>
                    ...
                    {DAY_LABELS.map((d, i) => (
                      <button key={i} onClick={() => handleDayToggle(i)}
                        className={cn(
                          ...
                          dayToggles[i] && 'text-white',
                          i === todayIdx && !dayToggles[i] && 'border-2',
                          !dayToggles[i] && i !== todayIdx && 'bg-[var(--sl-s3)] text-[var(--sl-t3)]',
                        )}
                        style={{
                          ...(dayToggles[i] ? { background: 'var(--sl-em)' } : {}),
                          ...(i === todayIdx && !dayToggles[i] ? { borderColor: MOD_FUTURO, color: MOD_FUTURO } : {}),
                        }}
                      >
                        {d}
                      </button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="flex-1 px-3 py-2 bg-[var(--sl-s2)] border border-[var(--sl-border)] rounded-[9px]
                               text-[var(--sl-t1)] text-[13px] sl-num outline-none
                               transition-colors focus:border-[var(--sl-border-em)]"
                  />
                  {goal.inputSuffix && (
          ...
          <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[16px] p-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)] mb-2">
              Notas da semana
            </p>
            <textarea
              value={notes}
              ...
              className="w-full bg-[var(--sl-s2)] border border-[var(--sl-border)] rounded-[10px] p-3
                         ...
                         placeholder:text-[var(--sl-t3)] transition-colors focus:border-[var(--sl-border-em)]"
            />
          </div>

          {/* Mobile Save (CTA --sl-em sólido G-03) */}
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full py-3 rounded-[14px] text-[14px] font-semibold text-white disabled:opacity-60 flex items-center justify-center gap-2 hover:opacity-90 transition-opacity"
      ...
      <div className="hidden lg:block max-w-[1160px] mx-auto px-10 py-9 pb-16">
        ...
        <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[18px] p-6 px-7 mb-7 relative overflow-hidden
          ...
          <SectionHeader
            ...
            sub="Acompanhar emoções ajuda a entender o que motiva ou bloqueia seus objetivos."
            className="mb-4"
          />
          ...
          <div className="relative w-full h-[44px] rounded-[22px] overflow-hidden my-4">
            <div
              ...
              style={{ background: 'linear-gradient(90deg, rgba(219,100,120,0.25), rgba(217,150,46,0.25), rgba(31,166,122,0.25))' }}
            />
            <input
              type="range"
              min={1}
            ...
            >
              {mood <= 3
                ? <Frown size={18} className="text-[var(--sl-danger)]" />
                : mood <= 6
                  ? <Meh size={18} className="text-[var(--sl-warning)]" />
                  : <Smile size={18} className="text-[var(--sl-success)]" />
              }
            </div>
          </div>
          ...
          <div className="flex justify-between text-[11px] text-[var(--sl-t3)]">
            <span className="flex items-center gap-1">
              <Frown size={14} className="text-[var(--sl-danger)]" />
              Ruim
            </span>
            <span className="flex items-center gap-1">
              <Meh size={14} className="text-[var(--sl-warning)]" />
              Neutro
            </span>
            <span className="flex items-center gap-1">
              <Smile size={14} className="text-[var(--sl-success)]" />
              Ótimo
            </span>
          </div>
        </div>

        {/* ── 02 · 2x2 GOAL UPDATE GRID ── */}
        <div className="mb-3">
          <SectionHeader
            eyebrow="02 · ATUALIZAÇÕES"
            title="Atualize seus objetivos"
            sub="Registre o progresso desta semana em cada meta ativa."
          />
        </div>
        <div className="grid grid-cols-2 gap-[14px] mb-7 sl-fade-up sl-delay-2 max-sm:grid-cols-1">
          {displayGoals.length === 0 ? (
            <div className="col-span-2 bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[18px] p-10 text-center">
              <p className="text-[13px] text-[var(--sl-t2)]">
                Você ainda não tem objetivos ativos. Crie um para começar a fazer check-ins.
              </p>
            </div>
          ) : displayGoals.map(goal => {
        ...
        {/* ── NOTES + HISTORY SPLIT ── */}
        <div className="grid grid-cols-2 gap-[14px] sl-fade-up sl-delay-3 max-lg:grid-cols-1">

          {/* Left: 03 · Notes + Preview + Save */}
          <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[18px] p-6 relative overflow-hidden
            ...
            <div className="absolute top-0 left-6 right-6 h-[2.5px] rounded-b-sm" style={{ background: MOD_FUTURO }} />

            <div className="mb-4 flex items-center gap-2">
              <FileText size={16} style={{ color: MOD_FUTURO }} />
              <SectionHeader
                eyebrow="03 · NOTAS"
                title="Notas e impressões"
              />
            </div>
            ...
            <div
              ...
              style={{
                background: MOD_FUTURO_SOFT,
                border: '1px solid var(--sl-border)',
              }}
            >
              <div className="text-[12px] font-semibold mb-2" style={{ color: MOD_FUTURO }}>
                Preview do check-in
              </div>
              <div className="flex gap-4 text-[12px] flex-wrap">
                ...
                <div>
                  ...
                  <span style={{ color: MOD_FUTURO }} className="sl-num">{updatedGoalsCount}</span>
                </div>
                <div>
                  <span className="text-[var(--sl-t3)]">Treinos: </span>
                  <span className="text-[var(--sl-success)] sl-num">{trainDaysCount}/4</span>
                </div>
              </div>
            </div>

            {/* Save button — CTA sólido --sl-em (G-03) */}
            <button
              onClick={handleSave}
              disabled={saving}
          ...
          <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[18px] p-6 relative overflow-hidden
            ...
            <div className="mb-4 flex items-center gap-2">
              <Clock size={16} style={{ color: MOD_FUTURO }} />
              <SectionHeader
                eyebrow="04 · HISTÓRICO"
                title="Check-ins anteriores"
              />
            </div>

            <div className="flex flex-col gap-[6px]">
              {MOCK_HISTORY.length === 0 ? (
                <p className="text-[12px] text-[var(--sl-t3)] py-6 text-center">
