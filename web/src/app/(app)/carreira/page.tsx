...
import {
  Briefcase, Star, Clock, BarChart3, CheckCircle2, Flame, Map,
} from 'lucide-react'
import { cn } from '@/lib/utils'
import {
  useCarreiraDashboard, useCareerHistory, useSaveSkill, useAddHistoryEntry,
  SKILL_LEVEL_LABELS,
...
export default function CarreiraPage() {
  ...
  const skillBarsData = skills.slice(0, 6).map(s => ({
    ...
    color: s.category === 'hard_skill' ? CARREIRA
      : s.category === 'soft_skill' ? 'var(--sl-em)'
      : s.category === 'language' ? 'var(--sl-warning)'
      : s.category === 'certification' ? '#8B7BD4'
      : CARREIRA,
  }))
  ...
  const xpPct = Math.round(CARREIRA_XP.currentXp / CARREIRA_XP.nextLevelXp * 100)
  ...
  return (
    ...
    <div className="hidden lg:block max-w-[1160px] mx-auto px-10 py-9 pb-16">
      ...
      {!loading && !profile && (
        <div className="py-10 text-center">
          <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center"
               style={{ background: 'rgba(219,100,120,.08)' }}>
            <Briefcase size={28} style={{ color: CARREIRA }} />
          </div>
            ...
            {[
              ...
              <div key={step.n} className={cn(
                ...
                <div className={cn(
                  ...
                  step.active ? 'text-white' : 'bg-[var(--sl-s3)] text-[var(--sl-t2)]'
                )} style={step.active ? { background: CARREIRA } : undefined}>
                  {step.n}
                </div>
                <div>
      ...
      {/* HERO: CAPITULO ATUAL · unico hero da tela (G-05) */}
        ...
        <div className="flex items-center gap-6 max-lg:flex-col max-lg:items-start">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-[10px] mb-[10px] flex-wrap">
              <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)]">
                Capitulo atual
              </span>
              <span
                className="inline-flex items-center gap-1 px-[10px] py-1 rounded-lg text-[11px] font-semibold"
                style={{ background: 'rgba(219,100,120,0.10)', color: CARREIRA }}
              >
                Level {CARREIRA_XP.level}
              </span>
              <span
                className="inline-flex items-center gap-1 px-[10px] py-1 rounded-lg text-[11px] font-semibold"
                style={{ background: 'rgba(217,150,46,0.10)', color: 'var(--sl-warning)' }}
              >
                <Flame size={11} />
              ...
              <div className="flex-1 max-w-[220px]">
                <div className="h-1 bg-[var(--sl-s3)] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${xpPct}%`, background: CARREIRA }}
                  />
                </div>
              </div>
              <span className="sl-num text-[11px] text-[var(--sl-t2)]">
          ...
          <div className="flex border border-[var(--sl-border)] rounded-[14px] overflow-hidden shrink-0 min-w-0">
            <div className="flex-1 min-w-0 px-[22px] py-4 text-center border-r border-[var(--sl-border)]">
              <div className="font-[DM_Sans] text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)] mb-1">
                Salario
              </div>
              <div className="sl-num-strong text-[22px] leading-none text-[var(--sl-em)] truncate">
                {salaryLabel}
              </div>
              {history.length > 0 && history[0].salary && profile?.gross_salary && history[0].salary !== profile.gross_salary && profile.gross_salary > history[0].salary && (
                <div className="text-[10px] text-[var(--sl-em)] mt-[2px]">
            ...
            <div className="flex-1 min-w-0 px-[22px] py-4 text-center border-r border-[var(--sl-border)]">
              ...
              <div className="sl-num-strong text-[22px] leading-none" style={{ color: 'var(--sl-warning)' }}>
                {roadmapProgress}%
              </div>
            ...
            <div className="flex-1 min-w-0 px-[22px] py-4 text-center">
              ...
              <div className="sl-num-strong text-[22px] leading-none text-[var(--sl-t1)]">
                {skills.length}
              </div>
              <div className="text-[10px] text-[var(--sl-t3)] mt-[2px]">
        ...
        text={
          profile
            ? <>Voce e <strong className="text-[var(--sl-t1)]">{profile.current_title ?? 'profissional'}</strong>
              {activeRoadmap && <> com <strong style={{ color: 'var(--sl-warning)' }}>{roadmapProgress}%</strong> do caminho para <strong className="text-[var(--sl-t1)]">{activeRoadmap.target_title}</strong> percorrido</>}.
      ...
      {loading ? (
        <div className="grid grid-cols-[1.4fr_1fr] gap-[14px] max-lg:grid-cols-1">
          <div className="h-80 rounded-[16px] bg-[var(--sl-s2)] animate-pulse" />
          <div className="h-80 rounded-[16px] bg-[var(--sl-s2)] animate-pulse" />
        </div>
      ) : error ? (
        ...
        <div className="grid grid-cols-[1.4fr_1fr] gap-[14px] max-lg:grid-cols-1 sl-fade-up sl-delay-2">

          {/* LEFT: Active Roadmap */}
          <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[16px] p-6 transition-colors hover:border-[var(--sl-border-h)]">
            <div className="flex items-center gap-[9px] font-[Syne] font-bold text-[15px] text-[var(--sl-t1)] mb-[18px] flex-wrap">
            ...
            {activeRoadmap ? (
              <>
                <p className="text-[12px] text-[var(--sl-t3)] mb-[18px]">
                  ...
                  {activeRoadmap.target_salary && ` · Salario alvo: ${fmtBRL(activeRoadmap.target_salary)}`}
                </p>
                ...
                {nextStep && (
                  <div className="mt-5 px-[18px] py-[14px] bg-[var(--sl-s2)] rounded-xl flex items-center gap-3">
                    ...
                    >
                      <CheckCircle2 size={16} style={{ color: CARREIRA }} />
                    </div>
                    <div className="flex-1 min-w-0">
              ...
              <div className="text-center py-8">
                <Map size={36} className="mx-auto mb-3 text-[var(--sl-t3)]" />
          ...
          <div className="flex flex-col gap-[14px]">
            {/* Top Skills card */}
            <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[16px] p-6 flex-1 transition-colors hover:border-[var(--sl-border-h)]">
              <div className="flex items-center gap-[9px] font-[Syne] font-bold text-[15px] text-[var(--sl-t1)] mb-[18px]">
                <Star size={16} style={{ color: CARREIRA }} />
                Top habilidades
                <span
                  className="ml-auto font-sans text-[12px] font-medium cursor-pointer hover:underline"
                  style={{ color: CARREIRA }}
              ...
              {skills.length === 0 ? (
                ...
                <SkillBars skills={skillBarsData} accentColor={CARREIRA} />
              )}
            </div>

            {/* Simulador CTA */}
            <div
              className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[16px] p-6 transition-colors hover:border-[var(--sl-border-h)] cursor-pointer"
            ...
            >
              <div className="flex items-center gap-[14px]">
                ...
                >
                  <BarChart3 size={20} style={{ color: CARREIRA }} />
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[12px] text-[var(--sl-t2)]">
                    ...
                    <span className="sl-num text-[var(--sl-em)] font-semibold">
                      +{fmtBRL(estimatedImpact)}/mes
                ...
                <button
                  className="inline-flex items-center gap-[7px] px-[18px] py-2 rounded-[11px] text-[12px]
                             font-semibold text-white hover:brightness-110 transition-all shrink-0"
                  style={{ background: 'var(--sl-em)' }}
                  onClick={(e) => {
                    e.stopPropagation()
