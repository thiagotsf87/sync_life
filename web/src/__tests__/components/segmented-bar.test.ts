...
describe('SegmentedBar', () => {
  describe('getSegments', () => {
    it('fills correct number of segments for level=3, maxLevel=5', () => {
      const segments = getSegments({ level: 3, maxLevel: 5, color: '#DB6478' })
      expect(segments).toHaveLength(5)
      expect(segments.filter(s => s.filled)).toHaveLength(3)
    })

    it('fills all segments when level equals maxLevel', () => {
      const segments = getSegments({ level: 5, maxLevel: 5, color: '#0F766E' })
      expect(segments.every(s => s.filled)).toBe(true)
    })

    it('fills no segments when level is 0', () => {
      const segments = getSegments({ level: 0, maxLevel: 5, color: '#DB6478' })
      expect(segments.every(s => !s.filled)).toBe(true)
    })

    it('uses provided color for filled segments', () => {
      const segments = getSegments({ level: 2, maxLevel: 5, color: '#0B2D34' })
      expect(segments[0].background).toBe('#0B2D34')
      expect(segments[1].background).toBe('#0B2D34')
      expect(segments[2].background).toBe('var(--sl-s3)')
    })

    it('respects custom maxLevel', () => {
      const segments = getSegments({ level: 3, maxLevel: 10, color: '#DB6478' })
      expect(segments).toHaveLength(10)
      expect(segments.filter(s => s.filled)).toHaveLength(3)
    })

    it('works with maxLevel=1', () => {
      const segmentsFilled = getSegments({ level: 1, maxLevel: 1, color: '#DB6478' })
      expect(segmentsFilled).toHaveLength(1)
      expect(segmentsFilled[0].filled).toBe(true)

      const segmentsEmpty = getSegments({ level: 0, maxLevel: 1, color: '#DB6478' })
