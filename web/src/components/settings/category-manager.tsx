...
// Paleta v3 — cores dessaturadas
const PRESET_COLORS = [
  '#DB6478', '#D97534', '#D9962E', '#1FA67A',
  '#0F766E', '#14b8a6', '#3CA0B5', '#4F88D4',
  '#6B6FD4', '#8B7BD4', '#C76795', '#6F7986',
]
...
export function CategoryManager() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      showToast('Preencha o nome da categoria', 'err')
      return
    }
    setIsSaving(true)
    try {
  ...
  return (
    <>
      <div className="flex flex-col gap-4">
        {/* 01 · Filtros */}
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--sl-em)] mb-2 font-[Syne]">
            01 · TIPO DE CATEGORIA
          </p>
          <div className="flex gap-2">
            <button
              ...
              className={cn(
                ...
                activeTab === 'expense'
                  ? 'bg-[rgba(219,100,120,0.12)] text-[var(--sl-danger)] border-[rgba(219,100,120,0.3)]'
                  : 'bg-transparent text-[var(--sl-t2)] border-[var(--sl-border)] hover:border-[var(--sl-border-h)]',
              )}
            >
              <ArrowUpCircle size={13} /> Despesas
            </button>
            <button
              ...
              className={cn(
                'flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[12px] font-semibold border transition-all',
                activeTab === 'income'
                  ? 'bg-[rgba(15,118,110,0.12)] text-[var(--sl-em)] border-[rgba(15,118,110,0.3)]'
                  : 'bg-transparent text-[var(--sl-t2)] border-[var(--sl-border)] hover:border-[var(--sl-border-h)]',
              )}
            >
              <ArrowDownCircle size={13} /> Receitas
            </button>
          </div>
        </div>

        {/* 02 · Grid de categorias */}
        <SLCard hover className="!p-5">
          <SectionHeader
            eyebrow="02 · CATEGORIAS"
            title={activeTab === 'expense' ? 'Categorias de despesa' : 'Categorias de receita'}
            sub={`${defaultsForTab.length} padrão · ${customForTab.length} personalizadas`}
            className="mb-4"
          />
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            {defaultsForTab.map((cat) => (
              ...
              >
                ...
                <span className="text-[12px] font-medium text-[var(--sl-t1)] flex-1 min-w-0 truncate">
                  {cat.name}
                </span>
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-[var(--sl-s3)] text-[var(--sl-t3)] shrink-0 uppercase">
                  Padrão
                </span>
              </div>
            ))}
            ...
            {customForTab.map((cat) => (
              <div
                key={cat.id}
                className="group flex items-center gap-2.5 p-2.5 bg-[var(--sl-s2)] rounded-xl border border-dashed border-[var(--sl-border-h)] transition-colors hover:border-[var(--sl-border-em)]"
              >
                <div
                  className="w-8 h-8 rounded-[9px] flex items-center justify-center text-base shrink-0"
                  style={{ background: cat.color + '22' }}
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                  ...
                  <button
                    ...
                    className="p-1 rounded-md text-[var(--sl-t3)] hover:text-[var(--sl-danger)] hover:bg-[var(--sl-s3)] transition-colors disabled:opacity-40"
                  >
                    <Trash2 size={12} />
                  </button>
                </div>
            ...
            <button
              onClick={openNewForm}
              className="flex items-center justify-center gap-2 p-2.5 rounded-xl border border-dashed border-[var(--sl-border-h)] text-[var(--sl-t3)] hover:text-[var(--sl-em)] hover:border-[var(--sl-border-em)] hover:bg-[rgba(15,118,110,0.04)] transition-all text-[12px] font-medium min-h-[52px]"
            >
              <Plus size={14} />
              Nova categoria
            </button>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    <span className="inline-flex items-center justify-center gap-1.5">
                      <ArrowDownCircle size={12} /> Receita
                    </span>
                  </button>
                </div>
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <DangerZone title="EXCLUIR CATEGORIA" className="!p-5">
              ...
              <div className="flex items-center gap-2.5 p-2.5 bg-[var(--sl-s2)] rounded-xl border border-[var(--sl-border)] mb-4">
                <div
                  className="w-8 h-8 rounded-[9px] flex items-center justify-center text-base shrink-0"
              ...
              {deleteConfirm.usageCount !== null && deleteConfirm.usageCount > 0 ? (
                ...
                >
                  ...
                  <p className="text-[12px] text-[var(--sl-t2)] leading-snug font-[DM_Sans]">
                    ...
                    {deleteConfirm.usageCount === 1 ? 'transação' : 'transações'}. Ao excluir, elas ficarão sem categoria.
                  </p>
                </div>
              ) : (
                <p className="text-[12px] text-[var(--sl-t3)] mb-4 font-[DM_Sans]">
