    ...
    <div className="flex-1 bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[18px] p-6 sl-fade-up sl-delay-3 hover:border-[var(--sl-border-h)] transition-colors">
      ...
      <div className="grid grid-cols-7 gap-1.5 text-center mb-3.5">
        {weekDays.map((wd, i) => {
          ...
          const dotColor = wd.isToday ? '#6B6FD4' : hasEvents ? (getEventTypeInfo(wd.events[0].type)?.color ?? '#3CA0B5') : undefined
          return (
            <div key={i}
              className={cn(
                'flex flex-col items-center gap-1 py-2 px-1 rounded-[10px] cursor-pointer transition-colors',
                wd.isToday && 'bg-[rgba(107,111,212,.08)] border border-[rgba(107,111,212,.2)]'
              )}>
              <span className={cn('text-[10px] font-semibold', wd.isToday ? 'text-[#6B6FD4]' : 'text-[var(--sl-t3)]')}>{DAY_NAMES[i]}</span>
              <div
                className="w-2 h-2 rounded-full mb-1"
                style={{
                  background: dotColor ?? 'var(--sl-s3)',
                  boxShadow: wd.isToday && dotColor ? `0 0 8px rgba(107,111,212,.4)` : undefined,
                }}
              />
              <span className={cn('font-[IBM_Plex_Mono] text-[11px]', wd.isToday ? 'text-[#6B6FD4]' : hasEvents ? 'text-[var(--sl-t2)]' : 'text-[var(--sl-t3)]')}>
          ...
          : events.slice(0, 4).map((ev, i) => {
              ...
              return (
                <div key={ev.id}
                  ...
                  <span className="font-[IBM_Plex_Mono] text-[11px] text-[var(--sl-t3)] w-8 flex-shrink-0">{dayLabel}</span>
