...
import { EXPENSE_CATEGORIES, INCOME_CATEGORIES, resolveCategory } from '@/constants/categories'
import { useUserCategories } from '@/hooks/use-user-categories'
import { formatCurrencyInput, formatDate } from '@/lib/format'
import { createClient } from '@/lib/supabase/client'
import { toast } from 'sonner'
import { ArrowDown, ArrowUp, Check, RefreshCw, AlertTriangle } from 'lucide-react'
...
export function TransactionForm({
  ...
  const [value, setValue] = useState(
    editTransaction
      ? formatCurrencyInput(String(Math.round(Number(editTransaction.amount) * 100)))
      : ''
  )
  ...
  const categories = [...defaultCats, ...customCategories.filter(c => c.type === type)]
  ...
  const handleSubmit = () => {
    if (!validate()) return
    if (editTransaction) {
      // Edição: mostrar confirmação antes de salvar
    ...
    try {
      ...
      if (editTransaction) {
        ...
        const { error } = await (supabase as any)
          .from('transactions')
          .update({ type, description, amount: numericValue, category_key: categoryId, date })
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="bg-slate-900 border-slate-800 sm:max-w-lg lg:max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="space-y-6 py-4 overflow-y-auto flex-1 min-h-0 pr-1">
            ...
            {/* Recurrence toggle */}
            <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-xl border border-slate-700 opacity-50 cursor-not-allowed">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Em breve — repetição automática mensal</p>
                </div>
              </div>
              <button
                type="button"
                disabled
                className="relative w-11 h-6 rounded-full bg-slate-700 cursor-not-allowed"
              >
                <span className="absolute top-[2px] left-[2px] w-5 h-5 rounded-full bg-slate-400" />
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-slate-800 flex-shrink-0">
            ...
            >
              ...
              {editTransaction ? 'Salvar alterações' : 'Salvar transação'}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
            ...
            <div className="bg-slate-800 rounded-xl p-4 space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-medium px-2 py-0.5 rounded-md text-xs ${
                  type === 'income' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-rose-500/15 text-rose-400'
                }`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-white font-medium truncate max-w-[160px]">{description}</span>
              </div>
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-white">{date ? formatDate(date) : '—'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400">Categoria</span>
                <span className="text-white flex items-center gap-1.5">
                  <span>{resolveCategory(categoryId, customCategories).icon}</span>
                  <span className="font-medium">{resolveCategory(categoryId, customCategories).name}</span>
                </span>
              </div>
            </div>
          </div>
