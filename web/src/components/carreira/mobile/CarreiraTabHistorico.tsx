...
export function CarreiraTabHistorico({ history, onAddPromotion }: CarreiraTabHistoricoProps) {
  ...
  return (
    <div>
        ...
        <div className="flex items-end gap-1 h-[60px] mb-2">
          {MOCK_SALARY_DATA.map((d, i) => (
            <div
              ...
              style={{
                height: `${d.heightPct}%`,
                background: d.heightPct >= 64 ? grad : `rgba(139,123,212,${d.heightPct >= 40 ? '0.35' : '0.2'})`,
              }}
            />
          ))}
        </div>
      ...
      {/* Coach */}
      <div className="mx-4 mt-3 mb-3 flex gap-[11px] p-[13px_14px] rounded-2xl border border-[rgba(139,123,212,0.25)]"
