    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-[520px] max-h-[90vh] flex flex-col bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          {/* Frequência */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)]">Frequência</label>
              ...
              {FREQ_OPTIONS.map(f => (
                <button key={f.value} onClick={() => setFrequency(f.value)}
                  className={cn(
                    ...
                    frequency === f.value
                      ? 'bg-[var(--sl-em-soft)] border-[var(--sl-em)] text-[var(--sl-em)]'
          ...
          <div className="grid grid-cols-2 gap-3">
            {showDayOfMonth && (
              <div className="flex flex-col gap-1.5">
                <label className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)]">Dia do mês</label>
                <input type="number" min={1} max={31} value={dayOfMonth}
                  onChange={e => setDayOfMonth(Math.max(1, Math.min(31, Number(e.target.value))))}
                  className="w-full px-3.5 py-2.5 rounded-[10px] bg-[var(--sl-s2)] border border-[var(--sl-border)] text-[13px] text-[var(--sl-t1)] outline-none focus:border-[var(--sl-em)] transition-colors sl-num" />
              </div>
            )}
            <div className="flex flex-col gap-1.5">
              <label className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)]">Data de início</label>
              <DatePickerInput
                value={startDate}
                onChange={setStartDate}
                error={!!errors.startDate}
              />
              {errors.startDate && <p className="text-[11px] text-[var(--sl-danger)]">{errors.startDate}</p>}
            </div>
          </div>

          {/* Encerramento */}
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--sl-t3)]">Encerramento (opcional)</label>
            <DatePickerInput
              value={endDate}
              onChange={setEndDate}
            ...
            />
            {errors.endDate && <p className="text-[11px] text-[var(--sl-danger)]">{errors.endDate}</p>}
          </div>
