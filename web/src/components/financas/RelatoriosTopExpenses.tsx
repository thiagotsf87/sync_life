...
import { Trophy, ArrowDownRight } from 'lucide-react'
        ...
        <div className="flex flex-col gap-1">
          {topExpenses.map((txn, i) => (
            <div
              ...
              <span className="sl-num text-[10px] text-[var(--sl-t3)] w-3.5 text-center shrink-0">{i + 1}</span>
              <span className="text-[14px] shrink-0 text-[var(--sl-t3)]">{txn.categories?.icon ?? <ArrowDownRight size={14} />}</span>
              ...
              <span className="sl-num text-[13px] text-[#DB6478] shrink-0">{fmtR(txn.amount)}</span>
              <span className="font-[IBM_Plex_Mono] text-[10px] text-[var(--sl-t3)] shrink-0">{formatDate(txn.date)}</span>
            </div>
          ))}
        </div>
      )}
