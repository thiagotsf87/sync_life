    ...
    <div className="grid gap-3 mb-3" style={{ gridTemplateColumns: '1fr 400px' }}>
      ...
      <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[14px] p-4 hover:border-[var(--sl-border-h)] transition-colors">
        <div className="flex items-center justify-between mb-4">
          <p className="font-[Space_Grotesk] font-bold text-[13px] text-[var(--sl-t1)]">Histórico — Receitas vs Despesas</p>
        </div>
        <div className="flex gap-1.5 items-start">
          <div className="flex flex-col justify-between text-right" style={{ height: 130, paddingBottom: 24 }}>
            {['7k', '5k', '3k', '1k'].map(v => (
              <span key={v} className="font-[IBM_Plex_Mono] text-[9px] text-[var(--sl-t3)] leading-none">{v}</span>
            ))}
          </div>
          <div className="flex-1 min-w-0">
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={histData} barCategoryGap="20%" barGap={2}>
                ...
                <Bar dataKey="rec" fill="#0F766E" fillOpacity={0.75} radius={[3, 3, 0, 0]} />
                <Bar dataKey="des" fill="#DB6478" fillOpacity={0.65} radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div className="flex gap-3 mt-2 pt-2 border-t border-[var(--sl-border)]">
          {[{ cor: '#0F766E', label: 'Receitas' }, { cor: '#DB6478', label: 'Despesas' }].map(l => (
      ...
      <div className="bg-[var(--sl-s1)] border border-[var(--sl-border)] rounded-[14px] p-4 hover:border-[var(--sl-border-h)] transition-colors">
        <div className="flex items-center justify-between mb-4">
          <p className="font-[Space_Grotesk] font-bold text-[13px] text-[var(--sl-t1)]">Gastos por Categoria</p>
          {alertCat && (
            <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-[rgba(217,150,46,0.12)] text-[#D9962E]">
            ...
            {catData.length === 0 ? (
              ...
              catData.map(cat => (
                <div key={cat.nome} className="flex items-center gap-2 py-1.5 border-b border-[var(--sl-border)] last:border-b-0">
                  ...
                  <span className="font-[IBM_Plex_Mono] text-[11px] text-[var(--sl-t3)]">R$ {fmtR$(cat.val)}</span>
